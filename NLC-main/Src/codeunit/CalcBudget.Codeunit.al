...
codeunit 52193442 CalcBudget
{
    ...
    begin
        ...
        if PurchHeader.FindFirst() then begin
            ...
            if BCSetup.Mandatory then//budgetary control is mandatory
              begin
                ...
                //BCSetup."Current Budget Start Date",BCSetup."Current Budget End Date");
                    ...
                    if (CheckDate > BCSetup."Current Budget End Date") then
                        exit;
                //ERROR('The Current Date %1 In The Order Does Not Fall Within Budget Dates %2 - %3',CheckDate,
                //BCSetup."Current Budget Start Date",BCSetup."Current Budget End Date");
                //Is budget Available
            ...
            if CheckType = CheckType::"G/L Account" then
                BudgetGl := CheckNo;
            ...
            if ExitOption = ExitOption::Available then
                ...
                if ExitOption = ExitOption::Budget then
                    exit(BudgetAmount)
                else
                    if ExitOption = ExitOption::Committment then
                        exit(CommittmentAmount)
                    else
                        if ExitOption = ExitOption::Actuals then
                            exit(ActualAmount)
        end
