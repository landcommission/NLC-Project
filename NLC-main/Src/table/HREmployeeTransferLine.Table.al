...
table 52193605 "HR Employee Transfer Line"
{
    ...
    fields
    {
        ...
        field(2; "Employee No"; Code[50])
        {
            ...
            begin
                ...
                EmpInfo.Reset();
                EmpInfo.SetRange(EmpInfo."No.", "Employee No");
                if EmpInfo.Find('+') then begin
                    "Employee Name" := Employees."First Name" + ' ' + Employees."Middle Name" + ' ' + Employees."Last Name";
                    "Global Dimension 1 Code" := EmpInfo."Global Dimension 1 Code";
                    "Global Dimension 2 Code" := EmpInfo."Global Dimension 2 Code";
                    "Current Grade" := EmpInfo."Salary Grade";
                    "Current Designation Name" := EmpInfo."Job Title";
                    "Responsibility Center" := EmpInfo."Responsibility Center";
                    "Current Designation" := EmpInfo."Job ID";
                    "Current Region Name" := EmpInfo."County Name";
                end
                else
                    if not EmpInfo.Find('-') then begin
                        ...
                        "Current Designation" := Employees."Job ID";
                        "Current Region Name" := Employees."County Name";
                        //"Shortcut Dimension 3 Code":=Employees."";
                ...
                TransHeader.Reset();
                TransHeader.SetRange(TransHeader."Request No", "Request No");
                TransHeader.SetFilter(TransHeader.Reason, '%1', TransHeader.Reason::"On Transfer");
                if TransHeader.Find('-') then begin
                    PRSalary.Reset();
                    PRSalary.SetRange(PRSalary."Employee Code", "Employee No");
                    if PRSalary.Find('-') then begin
                        "Transaction Code" := 'E11';
                        "Transaction Name" := 'Transfer Allowance';
                        Amount := Round(1 / 3 * PRSalary."Basic Pay", 0.01, '>');
                    end;
                end;
            end;
        }
    ...
    var
        ...
        EmpInfo: Record "HR Employees Info Changes";
}
