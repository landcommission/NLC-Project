...
report 52193578 "1/3 Basic Payroll Rule"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/13 Basic Payroll Rule.rdlc';
    ...
    dataset
    {
        dataitem("Bank Payment List"; "Bank Payment List")
        {
            ...
            column(ThirdBasicPay; ThirdBasicPay) { }
            ...
            begin
                ...
                PPeriod.Reset();
                PPeriod.SetRange(PPeriod."Employee Code", "Bank Payment List"."Staff No.");
                PPeriod.SetRange(PPeriod."Transaction Code", 'BPAY');
                PPeriod.SetRange(PPeriod."Payroll Period", "Bank Payment List"."Payroll Period");
                if PPeriod.FindLast() then
                    BasicPay := PPeriod.Amount;

                ThirdBasicPay := 1 / 3 * BasicPay;

                if "Bank Payment List".Amount > ThirdBasicPay then
                    CurrReport.Skip();
    ...
    var
        ...
        ThirdBasicPay: Decimal;
        PPeriod: Record "PR Period Transactions";
        BasicPay: Decimal;
}
