...
page 52193650 "Bank Pay In Slip Confirmation"
{
    DeleteAllowed = false;
    InsertAllowed = false;
    PageType = Document;
    ...
    SourceTableView = where(Status = const(Confirmation),
                            ...
                            Vaulted = const(false));
    ApplicationArea = All;
    Caption = 'Bank Pay In Slip Confirmation';
    layout
    {
        area(Content)
        {
        ...
        area(Processing)
        {
            action(Post)
            {
                ...
                begin
                    ...
                    Rec.CalcFields(Rec."Confirmed Amount");
                    if Rec."Confirmed Amount" = 0 then
                    ...
                    if BPL.Find('-') then
                        repeat
                            if BPL.Type = BPL.Type::Cheque then
                                if BPL."Cheque No" = '' then
                                    Error('Please ensure that the cheque number in all the deposit lines are entered');
                        until BPL.Next() = 0;

                    if LineAmount <> Rec."Confirmed Amount" then
            ...
            action("&Send Back")
            {
                ...
                Promoted = true;
                PromotedCategory = Process;
                ToolTip = 'Executes the &Send Back action.';
                trigger OnAction()
                begin
                    if Confirm('Do you wish to send the Bank Deposit Slip Back?', false) = false then
                        exit;
                    Rec.TestField(Rec."Return Remarks");
