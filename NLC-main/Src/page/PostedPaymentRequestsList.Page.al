...
page 52193690 "Posted Payment Requests List"
{
    ...
    SourceTableView = where("Payment Type" = const(Express),
                            Posted = const(true));
    ApplicationArea = All;
    Caption = 'Posted Payment Requests List';
    layout
    {
        area(Content)
        {
            ...
            group("<Action1102755006>")
            {
                ...
                action("<Action1102755007>")
                {
                    ...
                    begin
                        ...
                        //ApprovalMgt.SendPVApprovalRequest(Rec) then;
                    end;
                }
                action("<Action1102755028>")
                {
                    ...
                    begin
                        ...
                        //ApprovalMgt.CancelPVApprovalRequest(Rec, true, true) then;
                    end;
                }
    ...
    begin
        ...
        InvText := '';
        Appl.Reset();
        ///Appl.SETRANGE(Appl."Activity Code",Appl."Activity Code"::"0");
        Appl.SetRange(Appl."Activity Description", Rec."No.");
        Appl.SetRange(Appl."Account Type", LineNo);
        if Appl.FindFirst() then
            repeat
            ///InvText:=COPYSTR(InvText + ',' + Appl."Date Filter",1,50);
            until Appl.Next() = 0;
    end;
