...
page 50051 "Approved Requisitions"
{
    ...
    SourceTableView = where(Advertised = const(false), Closed = const(false), Status = filter(Approved));
    UsageCategory = Lists;
    Caption = 'HR Employee Requisitions List';
    layout
    {
        area(Content)
        {
    ...
    var
        myInt: Integer;
    //HRRequist: Record "HR Employee Requisitions";
    begin
        // GetAdvicedRequests();
        rec.SetFilter("Closing Date", '>%1', Today);

    end;

    local procedure GetAdvicedRequests()
    var
        myInt: Integer;
        HRRequist: Record "HR Employee Requisitions";
    begin
        HRRequist.Reset();
        HRRequist.SetFilter(HRRequist."Closing Date", '>%1', Today);
        HRRequist.SetRange(HRRequist.Advertised, true);
        HRRequist.SetRange(HRRequist.Closed, false);
        Page.Run(Page::"Advertised Requisitions", HRRequist);
